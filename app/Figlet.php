...
class Zend_Text_Figlet
{
    ...
    public function render($text, $encoding = 'UTF-8')
    {
        ...
        if ($textLength === false) {
            echo '$text is not encoded with ' . $encoding;
			exit;
        }
    ...
    protected function _loadFont($fontFile)
    {
        ...
        if (!file_exists($fontFile)) {
			echo $fontFile . ': Font file not found';
			exit;
        }
        ...
        if (substr($fontFile, -3) === '.gz') {
            if (!function_exists('gzcompress')) {
                echo 'GZIP library is required for '.'gzip compressed font files';
				exit;
            }
        ...
        if ($magic !== self::FONTFILE_MAGIC_NUMBER || $numsRead < 5) {
            echo $fontFile . ': Not a FIGlet 2 font file';
			exit;
        }
